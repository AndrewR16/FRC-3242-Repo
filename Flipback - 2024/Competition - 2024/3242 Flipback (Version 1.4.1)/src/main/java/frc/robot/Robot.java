...
public class Robot extends TimedRobot {
  ...
  // Intake Timer
  private final Timer intakeAutoStopTimer = new Timer();
  ...
  private boolean initialNoteView = false;
  ...
  public void teleopPeriodic() {
    ...
    // *Intake moves till a note is seen. Once it sees it activates a small delay
    // then stops. Outake functions as normal
    ...
    // TODO: Automatic intake stop
    if (intakeInf.get() == false) { // When a note is seen
    ...
    if (initialNoteView) {
      ...
      intakeAutoStopTimer.reset();
      intakeAutoStopTimer.start();
    }
    if (intakeAutoStopTimer.get() < Constants.intakeDelay) {
      m_intake.set(Constants.intakeSpeed);
    } else if (Controller.leftTrigger() == false && Controller.rightTrigger() == false && intakeIn == false
        && intakeOut == false) {
      m_intake.set(0);
    }
